...
sequenceDiagram
  ...
  F->>DB: 予約終了を検知（タイマー）→ status=completed
  Note over F,DB: 終了時刻に在車なら overstay（分岐表参照）
  F->>DB: 料金計算（予約枠×単価）→ Fee 記録
...
### ④ 料金確定（happy path）
本図は「終了時刻時点で既に出庫済み（空車）」の経路を示す。予約終了時刻の到来を Functions のタイマーが検知し、空車・終了済みなら予約を `completed` にして料金（予約枠 × 単価）を計算し Fee を記録する。終了時刻に在車のままなら `overstay` に分岐し、出庫後に完了＋超過料金を確定する（分岐表参照）。利用者はアプリから料金を確認できる。
