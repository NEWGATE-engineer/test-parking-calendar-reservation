...
# IoT Hub サービス接続文字列（gate-down の DOWN ダイレクトメソッド呼び出し用・ADR 0006）。
#   未設定ならローカルは notConfiguredDeviceCommandPort（gate-down 呼び出し時のみ失敗）になり、
#   IoT なしで起動・他機能の開発はできる。実デバイス連携を試す統合テスト/実環境でのみ設定する。
# IOT_HUB_CONNECTION_STRING=<IoT Hub 共有アクセスポリシー「service」の接続文字列>
# IOT_METHOD_TIMEOUT_SEC=30   # DOWN 応答待ち秒数。省略時既定 30（§12 未確定の仮値）
```

> この段階で必須なのは `SQL_CONNECTION_STRING`（ローカル Docker の SQL 接続）くらい。`JWT_SECRET` は認証を自前 JWT で実装する場合に使う秘密鍵で、認証方式が未確定（自前 JWT / Entra External ID）の現段階ではコメントアウトのままでよい（Entra External ID 採用時は不要）。`IOT_HUB_CONNECTION_STRING` は gate-down が実デバイスへ DOWN を送るための IoT Hub サービス接続文字列で、未設定ならローカルは自動的に未設定スタブで起動する（IoT 連携の統合テスト・実環境でのみ設定すればよい）。
...
```json
{
  ...
  "Values": {
    ...
    "SQL_CONNECTION_STRING": "Server=localhost,1433;Database=parking;User ID=sa;Password=Your_Strong_Passw0rd!;Encrypt=true;TrustServerCertificate=true;",
    "IOT_HUB_EVENTS": "<IoT Hub 組み込みエンドポイント（Event Hub 互換）の接続文字列>",
    "IOT_HUB_EVENT_HUB_NAME": "<同エンドポイントの Event Hub 名（既定 messages/events）>",
...
> `IOT_HUB_EVENTS` / `IOT_HUB_EVENT_HUB_NAME` は、テレメトリ受信（`onTelemetry` の Event Hub トリガ・[ADR 0005](../decisions/0005-telemetry-ingestion.md)）が IoT Hub の **組み込みエンドポイント（Event Hub 互換）** から入庫/出庫/UP を読むための設定です。IoT Hub ポータルの「組み込みのエンドポイント」から接続文字列と Event Hub 互換名を取得します。**DOWN 指示（ダイレクトメソッド invoke）に使うサービス接続文字列は backend 側の `IOT_HUB_CONNECTION_STRING`（8-2・[ADR 0006](../decisions/0006-iot-device-command-port.md)）**で、functions では使いません（テレメトリ受信と DOWN 送信は別経路）。
