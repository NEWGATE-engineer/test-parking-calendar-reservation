...
describe('availabilityForSpot', () => {
  ...
  it('競合なしは ok', () => {
    expect(availabilityForSpot({ ...base, conflicts: [] })).toEqual({
      available: true,
      reason: 'ok',
    });
  });
  ...
  it('時間帯が重複すれば reserved', () => {
    ...
    expect(availabilityForSpot({ ...base, conflicts: [c] })).toEqual({
      available: false,
      reason: 'reserved',
    });
  });
  ...
  it('buffer 近接と reserved が混在しても reserved が優先される（並び順に依存しない）', () => {
    const cBuffer = {
      start: new Date('2026-06-24T11:10:00Z'),
      end: new Date('2026-06-24T12:00:00Z'),
    }; // 近接
    const cReserved = {
      start: new Date('2026-06-24T10:30:00Z'),
      end: new Date('2026-06-24T11:00:00Z'),
    }; // 重複
